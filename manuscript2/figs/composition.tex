...
\begin{tikzpicture}
  ...
  \draw[Magenta, very thick] ($ (A0.south west) + (-.5,-.75) $) node [Magenta, below right] {Image Composition = MyComplexImage}
  rectangle ($ (A2.north east) + (.5,.25) $);

  \begin{scope}[on background layer]
    \fill[LightGray] ($ (A1.north west) + (-.5,.75) $) rectangle ($ (A3.south east) + (.5,-.25) $);
    \fill[DarkRed,opacity=0.25] ($ (A0.north west) + (-.5,.25) $) rectangle ($ (A2.south east) + (.5,-.75) $);
  \end{scope}
