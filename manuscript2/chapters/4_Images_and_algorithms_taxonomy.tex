...
TODO MDRange, OutputMDRange, ReversibleMDRange

\begin{table}[htbp]
  \begin{scriptsize}
    \texttt{Let \emph{MDRng}, \emph{OMDRng} and \emph{RMDRng} be types that model the concepts resp. \emph{MDRange},
      \emph{OutputMDRange} and \emph{ReversibleMDRange}. Let \emph{mdrng}, \emph{omdrng}, \emph{rmdrng} and \emph{val} be
    ...
    \begin{tabular}{llll}
      ...
      \thead{Concept}                                         & \thead{Expression}                          & \thead{Return Type}           & \thead{Description}                                                \\
      \cline{1-4}
      \multicolumn{1}{c|}{\multirow{2}{*}{MDRange}}           & \texttt{mdrng.begin()}                      & \texttt{Pix::reference\_type} & \makecell{Access the pixel's value for read and/or write purpose.} \\
      %\cline{2-4}
      \multicolumn{1}{c|}{}                                   & \texttt{mdrng.end()}                        & \texttt{Pix::point\_type}     & \makecell{Read the pixel's point.}                                 \\
      \cline{1-4}
      \multicolumn{1}{c|}{OutputMDRange}                      & \makecell{\texttt{auto it = omdrng.begin()}                                                                                                      \\ \texttt{*it++ = val}}        & \texttt{void}                 & Shift pixel's point coordinate base on \emph{pnt}'s coordinates.   \\
      \cline{1-4}
      \multicolumn{1}{c|}{\multirow{2}{*}{ReversibleMDRange}} & \texttt{rmdrng.rbegin()}                    & \texttt{void}                 & Mutate pixel's value.                                              \\
      \multicolumn{1}{c|}{}                                   & \texttt{rmdrng.rend()}                      & \texttt{Pix::point\_type}     & \makecell{Read the pixel's point.}                                 \\
      \cline{1-4}
    \end{tabular}
    \smallskip
...
\begin{table}[htbp]
  \begin{scriptsize}
    \texttt{Let \emph{pix}, \emph{opix}, \emph{pnt} and \emph{val} be instances of types resp. \emph{Pix}, \emph{OPix},
      \emph{Pix::point\_type} and \emph{Pix::value\_type}. Then we have the following valid expressions:}
