...
\end{minted}
\FIXME{The operator \texttt{Op} is instantiated with the correct dimension number. Then the \texttt{operator()}
  (parenthesis) is called while being forwarded the correct arguments.} In our case, it will instantiate the type
...
the baseline. This renews the interest there is to recompile the templated C++ library with an additional known type
than injecting it from Python long time running code. Being able to inject Python code ease prototyping and increase the
