...
we are operating on 8bits range (0-255) RGB via \texttt{'.r'}, \texttt{'.g'}, \texttt{'.b'} (l.9-11). Those hypotheses
...
% \begin{figure}[htbp]
%   \centering
%   \includegraphics[width=.5\linewidth]{figs/image_version}
%   \caption{Set of supported image type.}
%   \label{fig:image.version}
% \end{figure}
\begin{wrapfigure}{l}{0.4\textwidth}
  \centering
  \includegraphics[width=.38\textwidth]{figs/image_version}
  \caption{Set of supported image type.}
  \label{fig:image.version}
\end{wrapfigure}
...
image families as subtype. Indeed, an image with a LUT is subtype of the (global) Image type. This notion of sub-typing
is important because it may be abstracted behind an interface. A user can manipulate an image type without knowing its
specific subtype. This induces that the sub-typing facility may be handled internally in the library with dynamic
dispatching code (with run-time overhead). Each image processing library has its own way of handling sub-typing. More
generally, the model used to handle it is previously described in~\cref{subsec:different.approaches}. Indeed, subtypes
are handled the same way as their super-type but the fact that they have additional properties the algorithms can take
advantage of.

% \begin{figure}[htbp]
%   \centering
%   \includegraphics[width=.5\linewidth]{figs/fill_version_diagram}
...
versions in~\cref{fig:fill.version.diagram}.
...
\begin{wrapfigure}{r}{0.4\textwidth}
  \centering
  \includegraphics[width=.38\textwidth]{figs/fill_version_diagram}
  \caption{Diagram of the two versions of the fill algorithm.}
  \label{fig:fill.version.diagram}
\end{wrapfigure}
...
\end{minted}
  ...
  \begin{minipage}[r]{0.48\linewidth}
    \centering
    \includegraphics[scale=0.5]{figs/dilation_specialization_diagram}
  \end{minipage}
...
The figure~\ref{fig:image.specialization} shows how an algorithm specialization may exist in a set of algorithms
...
% \begin{figure}[htbp]
%   \centering
%   \includegraphics[width=.5\linewidth]{figs/image_version_specialization}
...
\begin{wrapfigure}{l}{0.4\textwidth}
  \centering
  \includegraphics[width=.38\textwidth]{figs/image_version_specialization}
  \caption{Algorithm specialization within a set.}
  \label{fig:image.specialization}
\end{wrapfigure}

Making a full inventory of images types is not possible as there are many families of image types, each family may
...
pixel requires to knowledge about what was previously computed for the previous pixels. Champfer distance transform,
...
\begin{figure}[htbp]
  \centering
  \subfloat[Dilation]{
    \includegraphics[width=1.7in]{figs/dilation_code}
  }%
  \hfil
  \subfloat[Erosion]{
    \includegraphics[width=1.7in]{figs/erosion_code}
  }%
...
Now we introduce an abstract way to represent this relation $Value \times Point$: the Pixel. This is a well known notion
in image processing, and it represents a couple $(point, value)$. This abstraction layer is easy to move around and
...
stored information: the point and the value. On top of that, the \emph{OutputPixel} can mutate the value. When
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=.8\linewidth]{figs/concepts/pixel}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=.8\linewidth]{figs/concepts/domain}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=.8\linewidth]{figs/concepts/image}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=.8\linewidth]{figs/concepts/images_all}
  \caption{All images concepts. \FIXME{Donner du sens aux flèches}}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=.8\linewidth]{figs/concepts/se_extension}
