...
known optimizations on periodic lines~\cite{vanherk.1992.localminmax}, it will be much faster. The dispatch diagram
...
pixel requires to know what was previously computed on the previous pixels as a prerequisite. Champfer distance
transform, labeling, watershed, hierarchy structures related algorithms are in this category.
...
and the new type \texttt{std::ptrdiff\_t} to store the result of a soustraction between two \texttt{std::size\_t}.
