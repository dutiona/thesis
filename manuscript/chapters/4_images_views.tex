...
\begin{minted}{C++}
  ...
  auto ret = ima1 + ima2;
\end{minted}
Is equivalent to writting the following code:
\begin{minted}{C++}
...
\paragraph{The logical operators} are implemented in the same way that arithmetical operators are. Both unary and binary
operators are expressed as transform views such as writting the following code:
\begin{minted}{C++}
  auto ret = !ima1 && ima2;
\end{minted}
is equivalent to writting the following code:
...
logic behind a ternary expression (if C then A else B). The rationale is to be able to swap between values depending on
a boolean mask. This way, a mathematical morphology algorithm such as \emph{hit or miss} can be implemented in the following simple manner:
\begin{minted}{C++}
...
l2norm, l2norm\_sqr, linfnorm, lpnorm, l0dist, l1dist, l2dist, l2dist\_sqr, linfdist, lpdist.
Calling an operator onto an image is equivalent to calling a transform view on each value of this image:
\begin{minted}{C++}
  auto ima = /* ... */;
