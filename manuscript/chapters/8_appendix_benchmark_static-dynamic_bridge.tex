...
namespace my
{
  ...
  mln::image2d<float> slow_stretch(const mln::image2d<T>& src, const mln::value_set<pybind11::object>& py_vs)
  {
    mln::image2d<float> res = mln::transform(src, [&py_vs](auto val) -> float {
      auto pymax = py_vs.max();
      auto pyval = pybind11::cast(val);
      auto pyret = py_vs.div(pyval, pymax);

      std::unique_ptr<float> ret = py_vs.template cast<float>(pyret);
  ...
  {
    auto                vs  = mln::concrete_value_set<T>{};
    mln::image2d<float> res = mln::transform(src, [&vs](auto val) -> float {
      auto max  = vs.max();
      auto fval = vs.template cast<float>(val);
      auto fmax = vs.template cast<float>(max);
      auto vs_f = mln::concrete_value_set<float>{};
      return std::any_cast<float>(vs_f.div(fval, fmax));
    });
    return res;
  }
  ...
  {
    auto                vs  = src.get_value_set();
    mln::image2d<float> res = mln::transform(src, [&vs](auto val) -> float {
      auto max  = vs.max();
      auto fval = vs.template cast<float>(val);
      auto fmax = vs.template cast<float>(max);
      auto vs_f = mln::value_set<float>{};
      return vs_f.div(fval, fmax);
    });
    return res;
  }
} // namespace my
