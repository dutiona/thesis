\documentclass[11pt,a4paper]{book}
%\documentclass[11pt,a4paper,draft]{book}
\usepackage[inner=3cm,textwidth=16cm,top=2.5cm,bottom=2.5cm]{geometry}
\usepackage[utf8]{inputenc}
\usepackage{lmodern}
\usepackage[T1]{fontenc}
% \usepackage[numbers]{natbib}
\usepackage[mode=buildmissing]{standalone}
\usepackage{tikz}
\usepackage{float}
%\usepackage{placeins}
%\usepackage{refcheck}
\usepackage{graphicx}
\usepackage{hyperref}
\usepackage{subcaption}
\usepackage{minted}
\usepackage{booktabs}
\usepackage{csquotes}
\usepackage{multirow}
\usepackage{tabularx}
\usepackage{tikz-uml}
\usepackage{threeparttable}
\usepackage[]{siunitx}
\usepackage{makecell}
\usepackage{graphbox}
\usepackage{diagbox}
\usepackage[draft]{fixme}
\usepackage{bibentry}
\usepackage{amsfonts}
\usepackage{amssymb}
\usepackage{amsmath}
\usepackage{mathabx}
\usepackage{cleveref}
\usepackage{datetime}
%\usepackage{svg}
\usepackage{lettrine}
\usepackage{siunitx}
%\usepackage{wrapfig}
%\usepackage[backend=biber,style=alphabetic,sorting=ynt]{biblatex}
%\usepackage[backend=biber,sorting=ynt,style=numeric]{biblatex}
%\usepackage[backend=biber,sorting=ynt,style=draft,backref=true]{biblatex}
\usepackage[backend=biber,
            defernumbers=true,
            sorting=ynt,
            style=numeric,
            %style=draft,
            backref=true]{biblatex}

\DeclareBibliographyCategory{cited}
\AtEveryCitekey{\addtocategory{cited}{\thefield{entrykey}}}
\addbibresource{../bibliography.bib}

% remove widows and orphans
\clubpenalty = 10000
\widowpenalty = 10000
\displaywidowpenalty = 10000

% This allows for an additional line-breaking pass with the amount of
% "tolerable" white space per line increased by 1em.
% Needed for overfull hbox in bibliography
% \emergencystretch=1em
\usepackage[final]{microtype}

\usepackage[section]{placeins}

\usepackage{epstopdf}
\epstopdfDeclareGraphicsRule{.tif}{png}{.png}{convert #1 \OutputFile}
\AppendGraphicsExtensions{.tif}

\usepackage{pifont}
\newcommand{\cmark}{\ding{51}}%
\newcommand{\xmark}{\ding{55}}%
\newcommand{\eqmark}{{\bf \(\approx\)}}

\definecolor{lightgray}{rgb}{0.83, 0.83, 0.83}
\definecolor{lightblue}{rgb}{0.68, 0.85, 0.9}
\definecolor{lightgreen}{rgb}{0.56, 0.93, 0.56}
\definecolor{thistle}{rgb}{0.85, 0.75, 0.85}

\newcommand{\FIXME}[1]{{\color{red}#1}}
\newcommand{\PLAGIAT}[1]{{\color{magenta}#1}}

%\FIXME{Homogénéiser "value-type" vs. "value type"}

% Reset chapter number after each part
% \makeatletter
% \@addtoreset{chapter}{part}
% \makeatother

%\renewcommand{\baselinestretch}{1}

%FIXME général : schéma concepts, sens des flèches, raffinage vers le bas ou vers le haut (héritage, losanges)

\usetikzlibrary{3d,calc,positioning}
\tikzumlset{font=\scriptsize\tt}
\setminted{fontsize=\scriptsize}

\renewcommand\UrlFont{\color{blue}\rmfamily}


\input{flyleaf.tex}


\begin{document}

\flyleaf

\cleardoublepage


\input{chapters/acknowledgement.tex}

\cleardoublepage

\input{chapters/abstracts.tex}

\cleardoublepage

\setcounter{figure}{0}
\setcounter{table}{0}

\chapter*{Long summary}
\label{chap:long_summary}
\input{chapters/0_long_summary_en.tex}

\cleardoublepage

\setcounter{figure}{0}
\setcounter{table}{0}

\chapter*{R\'{e}sum\'{e} long}
\label{chap:resume_long}
\input{chapters/0_long_summary_fr.tex}

\cleardoublepage

\setcounter{figure}{0}
\setcounter{table}{0}

\tableofcontents
\label{table.of.contents}

\listoffigures
\label{list.of.figures}

\listoftables
\label{list.of.tables}

\cleardoublepage


\part{Context and History of Generic programming}
\label{part:context.history}


\input{chapters/1_introduction.tex}

\cleardoublepage

\input{chapters/2_genericity.tex}

\cleardoublepage


\part{Applying Generic programming for Image processing in the static world}
\label{part:static.world}


\input{chapters/3_Images_and_algorithms_taxonomy.tex}

\cleardoublepage

\input{chapters/4_images_views.tex}

\cleardoublepage


\part{Bringing Generic programming to the dynamic world}
\label{part:dynamic.world}

\input{chapters/5_static-dynamic_bridge.tex}

\cleardoublepage

\input{chapters/6_conclusion_and_perspectives.tex}

\cleardoublepage


\part{Appendices}
\label{part:annexes}

\appendix

% This allows for an additional line-breaking pass with the amount of
% "tolerable" white space per line increased by 1em.
% Needed for overfull hbox in bibliography
\emergencystretch=1em

\chapter{References}
\label{chap:bibliography}

\printbibliography[category=cited,heading=bibintoc]

%\nocite{*}
%\printbibliography[category=cited,heading=bibintoc,title={References}]
%\printbibliography[notcategory=cited,heading=bibintoc,title={Further reading}]

\cleardoublepage

\chapter{List of Publications}
\label{chap:publications}

\section*{Conference Papers}

\fullcite{roynard.2019.rrpr} \\

\noindent\fullcite{roynard.2022.gpce} \\

\cleardoublepage

\input{chapters/7_appendix_concepts_and_archetypes.tex}

\input{chapters/8_appendix_benchmark_static-dynamic_bridge.tex}


\end{document}
