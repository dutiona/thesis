...
\begin{frame}[fragile]{Need of Genericity for image processing}
  \begin{figure}[htbp]
    ...
    \caption{Watershed algorithm applied to three different image types.~\footnote{\bibentry{levillain.2014.ciarp}}}
    %\vspace{-0.5cm}
...
\begin{frame}[fragile]{2nd approach: Generalization}
  ...
  \end{minted}
\end{frame}
% FIXME: x, y, z, t même modèle que slide Non-generic
% s16 et 17 entre 11 et 12
% unifier code s12 s13 avec get_iterator -> .pixels(), .get_pixel -> ima(p) etc.
% unifier .value() .val()
...
\begin{frame}[fragile]{4th approach: Parametric Polymorphism}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    % FIXME: highlight template <class InImage>
...
\begin{frame}[fragile]{Toward constrained genericity: Constraints so far}
  ...
  % FIXME: add code back on same slide
\end{frame}

% FIXME : toward constrained genericity
...
\begin{frame}[fragile]{Our Concepts for Image Processing: fundamentals}
  ...
  \end{minted}
\end{frame}
% Besoin algorithmique
% FIXME: écrire un vrai algorithme point-wise en pseudo-code pour justifier:
% for (p in ima.domain())
%   ima(p) = 42
% OU 
% for (pix in ima.pixels()) // 0 cost abstraction
%   pix.val() = 42
% 
...
% FIXME : IndexableImage, besoin pour trier les pixels
...
\begin{frame}[fragile]{Our Concepts for Image Processing: Advanced Image concept}
  ...
  \begin{figure}
    ...
    % FIXME: virer Index/ReversibleMdrange
    % FIXME ajouter un plan text + Writability à droite du schéma + en parler à l'oral
  \end{figure}
\end{frame}


\begin{frame}[fragile]{Our Concepts for Image Processing: Local algorithms}
  % FIXME: virer ce qui est lié à l'extension
  \begin{itemize}
...
\begin{frame}[fragile]{Our Concepts for Image Processing: Local algorithms}
  ...
  \begin{figure}
    ...
    % FIXME: rajouter lien se -> image via se(pix) qui récupère les pixels voisins (image connait les pixels/points
    % voisins)
    % se(p) -> range de points (faire les 2 interfaces)
    % avec la 2nd version (pixel) -> 0-overhead
  \end{figure}
\end{frame}
...
% FIXME: attention redondance entre 2 slides
...
% FIXME: insister sur les overload pour ROI
% FIXME: ajouter le protoytpe ?
